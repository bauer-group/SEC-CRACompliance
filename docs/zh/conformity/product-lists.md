...
- **关键（Annex IV）：** 当前根据 **Art. 32(3) CRA** 采用 EU 型式检验（Module B+C）或全面质量保证（Module H）。只有**当**委员会根据 **Art. 8(1) CRA** 通过授权法案指明相关产品时，保证级别为"实质性"或更高的欧洲网络安全证书 (EUCC) 才成为强制要求（截至 2026 年 6 月：尚未通过此类法案）。
...
关键产品**当前**通过标准合规评估程序进行评估——根据 Art. 32(3) CRA 采用 [EU 型式检验（Module B+C）](/zh/conformity/module-bc) 或 [全面质量保证（Module H）](/zh/conformity/module-h)。只有当委员会根据 Art. 8(1) CRA 通过授权法案后，保证级别为"实质性"或更高的[欧洲网络安全证书 (EUCC)](/zh/conformity/eucc) 才成为强制要求。

::: warning EUCC 仅有条件强制
下表中的"合规模块"列将 EUCC 列为**未来可能**的程序。**截至 2026 年 6 月，它并非强制要求**——只要不存在根据 Art. 8(1) CRA 通过的授权法案，Annex IV 产品即遵循 Module B+C 或 Module H（Art. 32(3) CRA）。
:::

| 编号 | 产品类别 | 描述 | 合规模块 |
|------|----------|------|----------|
| 1 | 带有安全盒的硬件设备（HSM、智能卡等） | 在受保护环境中执行加密操作的硬件安全模块 (HSM)、智能卡及类似设备 | Module B+C / H（EUCC 有条件*） |
| 2 | 智能卡读卡器 | 用于读取和处理智能卡数据以进行认证、签名或加密的设备 | Module B+C / H（EUCC 有条件*） |
| 3 | 机器人和机器人控制器的传感器和执行器组件（工业应用） | 用于工业应用中机器人和机器人控制器的传感器和执行器 | Module B+C / H（EUCC 有条件*） |
| 4 | Directive (EU) 2019/944 Art. 2(23) 意义上的智能电表 | 根据电力内部市场指令测量能源消耗的电子计量系统 | Module B+C / H（EUCC 有条件*） |
| 5 | 属于高级计量基础设施 (AMI) 的所有设备和软件组件 | 作为高级计量基础设施一部分的所有设备和软件 | Module B+C / H（EUCC 有条件*） |

*\* EUCC 仅在根据 Art. 8(1) CRA 通过授权法案后才成为强制要求；在此之前适用 Art. 32(3) CRA 规定的 Module B+C 或 H（截至 2026 年 6 月）。*
